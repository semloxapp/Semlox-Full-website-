...
function awbPayloadFromExtraction(extraction: AwbExtractionResponse) {
  const values = Object.fromEntries(
    extraction.fields.map((field) => [
      field.key,
      [field.value, field.confidence, field.needsReview] as const,
    ])
  );
  return {
...
function ExtractedFieldsPanel({
  ...
  const matchesFilter = (field: AwbExtractedField, selectedFilter: FieldFilter) => {
    if (selectedFilter === "review") {
      return field.status === "review" || field.needsReview || field.confidence === 0;
    }
    if (selectedFilter === "warning") {
      return field.status === "warning" || field.color === "amber";
    }
    if (selectedFilter === "valid") {
      return field.status === "valid" || field.color === "green";
    }
    if (selectedFilter === "reviewed") return manuallyReviewedKeys.has(field.key);
