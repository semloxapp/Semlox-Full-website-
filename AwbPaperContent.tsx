...
//       const res = await fetch("/api/awb/documents/{documentId}/pdf", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
...
function AwbFormInner({
  ...
  async function handleGenerateFromForm() {
    ...
    const outgoingMap: Record<string, string> = {
      ...
      "departure-airport": "airport_departure",
      "requested-flight": "flight_no",
      "shipment-referente-number": "awb_number",
      "reference-number": "reference_number",
      "shipment-referente-information": "reference_number2",
      "currency": "currency_value",
      "CHGS": "chgs_value",
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      // Use AWB number for filename (mapped from House_Airwaybill).
      const filenameCandidate = payload["awb_number"] || payload["reference_number"];
      const safeFilename =
        String(filenameCandidate || "awb")
          .replace(/[\\/:*?"<>|]+/g, "_")
          .trim() || "awb";
